...
public:
    ...
    void start() {
        _client.brokers(BROKER_ADDR, BROKER_PORT)
...
private:
    ...
    void subscribe_topics() {
        _client.async_subscribe(
            ...
            [this](boost::mqtt5::error_code ec, std::vector<boost::mqtt5::reason_code> rc, auto props) {
                if (!ec) std::cout << "[MQTT] Inscrito em: " << TOPIC_CMD << std::endl;
                else {
    ...
    void process_command(const std::string& json_str) {
        try {
            ...
            if (!json.contains("action")) return;
            ...
            if (action == "start") success = _manager.start_infusion();
            else if (action == "pause") success = _manager.pause_infusion();
            else if (action == "stop" || action == "abort") success = _manager.stop_infusion();
            ...
            // 3. Atalho: PURGE (Traduz para Config Mode 3)
                ...
                // Purge: Volume 0 (infinito/manual), Taxa Máxima, Modo 3
            ...
            // 4. Atalho: BOLUS (Traduz para Config Mode 2)
                ...
                // Tenta ler params específicos do bolus ou usa defaults seguros
                uint32_t vol = json.contains("volume") ? (uint32_t)json.at("volume").as_int64() : 5;   // Default 5ml
                uint32_t rate = json.contains("rate") ? (uint32_t)json.at("rate").as_int64() : 600;    // Default 600ml/h
                
                std::cout << "   -> Traduzindo Bolus para Config Mode 2" << std::endl;
                success = _manager.set_config(vol, rate, MODE_BOLUS);
            }

            if(success) std::cout << "   -> Comando Aceito (ACK)" << std::endl;
            else std::cerr << "   -> Comando Rejeitado (NACK)" << std::endl;
    ...
    void setup_manager_callbacks() {
        _manager.set_status_callback([this](const cmd_status_payload_t& status) {
            boost::asio::post(_io, [this, status]() {
                publish_status(status);
            });
        });
    }

    void publish_status(const cmd_status_payload_t& status) {
        boost::json::object j;
        j["state"] = status.current_state;
        j["volume_infused"] = status.volume;
        j["flow_rate"] = status.flow_rate_set;
        j["pressure"] = status.pressure;
        j["alarm"] = (status.alarm_active > 0);
